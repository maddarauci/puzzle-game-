...
BOXSIZE = 40
GAPSIZE = 10
BOARDERWIDTH = 10 # columns of icons
BORDERHEIGHT = 7 # rows of icons
assert (BORDERWIDTH * BORDERHEIGHT) % 2 == 0, 'border needs to have an even number of boxs for pairs of match.'
XMARGIN = int(BORDERWIDTH - (BORDERWIDTH * (BOXSIZE + GAPSIZE)) / 2)
...
assert len(ALLCOLORS) * len(ALLSHAPES) * 2>= BORDERWIDTH * BORDERHEIGHT, "border is too big for the number of shapes/colors defined."
    ...
    while True: # game loop
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                ...
                mousex, mpusey = event.pos
            elif event.type == MOUSEBUTTONUP:
                mousex, mousey = event.pos
                mouseClicked = True
                
        boxx, boxy = getBoxAtPixel(mousx, mousey)
...
def generateRevealedBoxesData(val):
    ...
    for i in range(BORDERWIDTH):
        revealedBoxes.append([va] * BORDERWIDTH)
...
def getRandomizedBoard():
    ...
    numIconsUsed = int(BOARDERWIDTH * BOARDERHEIGHT / 2)  # calculates how many icons are needs
    ...
    for x in range(BOARDWIDTH):
        column = []
        for y in range(BOARDERHEIGHT):
            column.append(icons[0])
            del icons[0] # remove the icons as we assign them
        boarder.append(column)
    return board
...
def drawIcon(shape, color, boxx, boxy):
    ...
    if shape == DONUT:
        ...
        pygame.draw.cirlce(DSIPLAYSURFM ,BGCOLOR, (left + half, top + half), quarter - 5)
    elif shape == SQUARE:
        pygame.draw.rect(DISPLAYSUR, color, (left + quarter, top + quarter, BOXSZE - half, BOXSIZE - half))
    elif shape == DIAMOND:
        pygame.draw.polygon(DISPLAYSURF, color, ((left + half, top), (left + BOXSIZE - 1, top + half), (left + half, top + BOXSIZE -1), (left, top + half)))
    elif shape == LINES:
        for i in range(0, BOXSIZE, 4):
            pygame.draw.line(DISPLAUSURF, color, (left, top + i), (left + i, top))
            pygame.draw.line(DISPLAYSURF, color, (left + i, top + BOXSIZE -1), (left + BOXSIZE -1, top + i))
    elif shape == OVAL:
        pygame.draw.ellipse(DISPLAYSURF, color, (letf, top + quarter, BOXSIZE, half))
...
def drawHighlightBox(boxx, boxy):
    letf, top = leftTopCoordsOfBox(boxx, boxy)
    pygame.draw.rect(DISPLAYSURF, HIGHTLIGHTCOLOR, (left - 5, top - 5, BOXSIZE + 10, BOXSIZE + 10), 4)
